...
async def recommend(item_id: str):
    ...
    # Cargar los DataFrames desde archivos parquet
    df = pd.read_parquet('datasets/dfgames.parquet')[['app_name', 'id']].head(n=27000)
    df_items = pd.read_parquet('datasets/users_item.parquet')[['user_id', 'item_id', 'playtime_forever']].sample(n=27000)
